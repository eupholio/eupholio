...
const recordTimeFormat = "2006-01-02"
...
func (e *Extractor) Execute(ctx context.Context, db boil.ContextExecutor, reader io.Reader, options ...eupholio.Option) error {
	...
	dists, err := extract(reader)
	if err != nil {
		return err
	}
	for _, tr := range dists {
...
func extract(reader io.Reader) (models.PoloniexDistributionSlice, error) {
	rows, err := extractRecords(reader)
	if err != nil {
		return nil, err
	}

	var dists models.PoloniexDistributionSlice
	...
	for _, row := range rows {
		...
		deposit := &models.PoloniexDistribution{
			ID:       0,
			Date:     date,
			Currency: row.Get(CurrencyColumn),
			Amount:   types.NewDecimal(amount),
			Wallet:   row.Get(WalletColumn),
		}
		dists = append(dists, deposit)
	}
